...
```c++
CHECK_THROWS_AS(formatInt(), const std::exception&);
CHECK_THROWS_AS(formatInt(), std::exception); // same as above
```
...
```c++
CHECK_THROWS_WITH(formatInt(), "invalid operation!");
```
...
```c++
CHECK_THROWS_WITH_AS(formatInt(), "invalid operation!", std::runtime_error);
```
...
```c++
REQUIRE_THROWS_WITH(formatInt(), doctest::Contains("Oopsie"));
```
